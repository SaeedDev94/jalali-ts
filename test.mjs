...
describe('Jalali', () => {
  describe('Create new instance', () => {
    ...
    describe("Parse date: '1398/12/04'", () => {
      ...
      const gregorian = '2020/02/23';
      it(`Same gregorian: '${gregorian}'`, () => assert.equal(jalali.format(dateFormat, true), gregorian));
      ...
      const add2MonthAnd1Week = '1399/02/11';
      it(`Add 2 month + 1 week: '${add2MonthAnd1Week}'`, () => assert.equal(jalali.add(2, 'month').add(1, 'week').format(dateFormat), add2MonthAnd1Week));

      const startOfWeek = '1399/02/06 00:00:00';
      it(`Start of week: '${startOfWeek}'`, () => assert.equal(jalali.startOf('week').format(dateTimeFormat), startOfWeek));

      const dayOfYear = 37;
      it(`Day of year: ${dayOfYear}`, () => assert.equal(jalali.dayOfYear(), dayOfYear));

      const endOfYear = '1399/12/30 23:59:59';
      it(`End of year: '${endOfYear}'`, () => assert.equal(jalali.endOf('year').format(dateTimeFormat), endOfYear));

      const is1399LeapYear = true;
      it(`Is leap year: ${is1398LeapYear}`, () => assert.equal(jalali.isLeapYear(), is1399LeapYear));

      const add1DayAndStartOfDay = '1400-01-01 12:00:00 AM';
      it(`Add 1 day + start of day: '${add1DayAndStartOfDay}'`, () => assert.equal(jalali.add(1, 'day').startOf('day').format('YYYY-MM-DD hh:mm:ss A'), add1DayAndStartOfDay));
    });
